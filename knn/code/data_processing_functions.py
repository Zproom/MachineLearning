...
# for the k-nearest neighbor algorithms.
...
def handle_missing_values(input_data, dataset):
    ...
    if dataset == "breast-cancer-wisconsin":
        ...
        average_ignoring_missing = input_data["bare_nuclei"].mean()
        # Replace missing values with the average for the column.
        input_data["bare_nuclei"] = input_data["bare_nuclei"].replace(np.nan, 
        average_ignoring_missing)
    return input_data
...
def handle_categorical_data(input_data, dataset):
    ...
    elif dataset == "car" or dataset == "house-votes-84":
...
def normalize_data(input_data, dataset):
    """This function normalizes numeric features using min-max normalization. The target column is 
    not normalized.
    ...
    for col in numeric_columns:
        input_data[col] = (input_data[col] - input_data[col].min()) / (input_data[col].max() - 
        input_data[col].min())
    return input_data
