...
def random_split(input_data, type):
    ...
    if type == "validation":
        ...
        validation_data = input_data.sample(frac = 0.2, random_state = 1)
    ...
    # Randomly split again, but split the original data into two halves.
    else:
        test_data = input_data.sample(frac = 0.5, random_state = 1)
        training_data = input_data.drop(test_data.index)
        test_data = test_data.reset_index(drop = True)
        training_data = training_data.reset_index(drop = True)
        return test_data, training_data

def stratify_classes(input_data, target_col):
    ...
    for label in stratify_labels:
        ...
        test_data_class = class_df.sample(frac = 0.5, random_state = 1)
    ...
    test_data = pd.concat(test_data).sample(frac = 1, random_state = 1).reset_index(drop = True)
    training_data = pd.concat(training_data).sample(frac = 1, random_state = 1).reset_index(drop = 
                                                                                            True)
